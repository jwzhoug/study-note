...
* 代码：
  ...
  该排序需要注意：当待排序记录的数量n 很小的时候这是一种很好的排序方法，但是通常待排序的记录都很大，所以直接使用插入排序就不恰当了，这时候在直接插入排序的基础上从***减少比较次数***+***减少记录移动次数***左手进行改进得出了下面两种排序方法
##### 2-分插入排序
  ...
  * 代码：
    ...
    static void BInsertionSort(int[] arr) {
            ...
            for (int i = 1; i < arr.length; i++) {
                ...
                // 然后开始进行插入排序 low 就是原有序序列中需要和待插入的值做交换最低位
...
该算法怎么推导出来的呢，从对直接插入排序的分析，最坏情况是O(n[^2]),若待排序序列为所期望的顺序序列

时间复杂度为O(n)，由此可以想到，若待排序序列**基本有序**，那么直接插入排序的效率就能得到较大的提升；
...
- 原理：从第一个记录开始，相邻记录作比较，如果前面一个大于后面一个（默认升序），那就交换位置，知道n-1这个位置和n这个位置的作为比较，第一趟下来最大的一个自然就在最后面的位置了，然后以此类推，进行n-1趟自然就排序完成了。
