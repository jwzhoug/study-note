...
* 使用 上面 CAS 方法的程序，如果在多处理器（多cpu）环境运行，就会为其加上 lock 前缀，带有这个前缀的该cas指令在现代的处理器中会使用缓存锁定并且搭配缓存一支新协议来保证这个变量的执行的原子性。
* 该指令的前后的读/写指令会被禁止重排序。
...
![1639076608379](https://github.com/Alan-Jun/study-note/blob/master/study-note.assets/1639076608379.png)
...
*  基于类初始化的解决方案
  ...
  **注意：线程优先级属性不能决定线程是否优先执行**
  ...
  ![1639168988013](https://github.com/Alan-Jun/study-note/blob/master/study-note.assets/1639168988013.png)
